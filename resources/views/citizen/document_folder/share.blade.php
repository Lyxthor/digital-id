...
<div class="container">
    ...
    <div>
        <div class="container mx-auto h-96">
            <div class="card bg-white rounded-md h-full">
                <table class="table table-xs">
                    <thead>
                      <tr>
                        <th class="py-5"></th>
                        <th class="border-r">Name</th>
                        <th class="border-r">Accessed</th>
                        <th class="border-r">Accessibility</th>
                        <th class="border-r">Active</th>
                        <th class="border-r">Expires at</th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex gap-2">
                                        ...
                                        <button
                                        onclick="delete_modal_{{ $token->id }}.showModal()"
                                        ...
                                        <dialog id="delete_modal_{{ $token->id }}" class="modal modal-bottom sm:modal-middle">
                                            <div class="modal-box">
                                                ...
                                                <form
                                                action="{{ route('citizen.token.destroy', ['id'=>$token->id]) }}"
                                                method="post">
                                                @method('DELETE')
                                                    @csrf
                                        ...
                                        <div class="modal-box">
                                            ...
                                            <form action="{{ route('citizen.token.update', ['id'=>$token->id]) }}" method="post">
                                                ...
                                                <div class="form-control mb-2">
                                                    ...
                                                    id="accessibility"
                                                    name="accessibility"
                                                    class="mt-1 select select-sm select-bordered w-full"
                                                    onchange="toggleCitizenContainer(event, {{ $token->id }})">
                                                        <option
                                                        value="public"
                                                        @selected($token->accessibility=='public')>
                                                            Public
                                                        </option>

                                                        <option
                                                        value="restricted"
                                                        @selected($token->accessibility=='restricted')>
                                                            Restricted
                                                        </option>
                                                    </select>
                                                </div>
                                                <div class="{{ $token->accessibility=='restricted' ? '' : 'hidden' }} w-full" id="authorized_citizen_container{{ $token->id }}">
                                                    ...
                                                    <ul
                                                    id="membersContainer{{ $token->id }}"
                                                    class="px-3 py-3 text-sm bg-base-200 rounded-md w-full mt-4">
                                                        ...
                                                        @foreach ($token->authorized_citizens as $c)
                                                            <li id="membershipList{{ $token->id }}_{{ $c->id }}" class="p-3 rounded-md">
                                                                <div class="flex items-center justify-between">
                                                                    ...
                                                                    <button type="button" class="btn btn-sm btn-error aspect-square rounded-sm" onclick="removeMemberFromList({{ $c->id }}, {{ $token->id }})">
                                                                            x
                                                                    </button>
                                                                </div>
                                                            </li>
                      ...
                      @else
                        <tr>
                            <td colspan="7" class="py-2 text-center bg-stone-100">
                                Belum ada token
                            </td>
                        </tr>
                      @endif
    ...
    function addMember(data,index,id)
    {
        ...
        if(check.length === 0)
        {
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="button" class="btn btn-sm btn-error aspect-square rounded-sm" onclick="removeMemberFromList(${data.id}, ${id})">
                        x
                </button>
            </div>`
